...
const detailTable = (req: Request, res: Response) => {
  ...
  header = [
      {title: '需求名称', field: 'name'},
      {title: '需求地区', field: 'area'},
      {title: '需求状态', field: 'status'},
      {title: '用户提交时间', field: 'userSubmitTime'},
      {title: '审核发布时间', field: 'reportTime'},
      {title: '认领时间', field: 'claimsTime'},
      {title: '分发时间', field: 'awayTime'},
      ...header
    ]
  ...
  const showPageTotal = (total - pageIndex * pageSize) > 0 ? pageSize : total - (pageIndex - 1) * pageSize
  ...
  res.json(resultFn({
    result: {
      ...
      pageIndex,
      pageSize,
      pageTotal: showPageTotal,
      totalCount: total,
      data: showPageTotal === 1 ? [data] : data
    }
  }))
}
