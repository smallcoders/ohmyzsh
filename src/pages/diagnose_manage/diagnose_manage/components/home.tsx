...
import { Link, history, Prompt, Access, useAccess } from 'umi';
import {
  getOrgTypeList,
  sortOrgType,
  removeOrgType
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        return (
          <Space size="middle">
            <Access accessible={access['P_DM_ZDGL']}>
              <a
                href="#"
                onClick={() => {
                  history.push(`/diagnose-manage/diagnose/add?id=${record?.id}&version=${record?.latestVersion}&firstQuestionnaireNo=${record?.firstQuestionnaireNo}`)
                }}
              >
                迭代{' '}
              </a>
        ...
        <div className="title">
          ...
          <Access accessible={access['P_DM_ZDGL']}>
            <Button
              ...
              key="addStyle"
              onClick={() => {
                history.push('/diagnose-manage/diagnose/add')
              }}
            >
              <PlusOutlined /> 新建诊断
            </Button>
