...
const LoginFC: React.FC = () => {
  const [userLoginState, setUserLoginState] = useState<Login.LoginResult>({success: true, message: ''});
  ...
  const handleSubmit = async (values: Login.LoginParam) => {
    ...
    try {
      ...
      const loginResult = await login({loginName, password: window.btoa(password), ticket: ticketRes.result});
