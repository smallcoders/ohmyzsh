...
export default () => {
  ...
  const handleSearch = (e: string) => {
    ...
    if (e.length > 0) {
      try {
        const data = {
          pageIndex: selectPageInfo.pageIndex,
          pageSize: selectPageInfo.pageSize,
          orgId,
          nameOrPhone: e,
        };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (audited: boolean) => {
        return audited ? <span>审核通过</span> : <span>未审核</span>;
      },
    },
    {
      title: '操作',
              ...
              : {
                  ...
                  showTotal: (total) =>
                    `共${total}条记录 第${pageInfo.pageIndex}/${
                      Math.ceil(pageInfo.total / pageInfo.pageSize) || 1
                    }页`,
                }
          }
        />
          ...
          <Select
            ...
            onPopupScroll={() => {
              if (selectPageInfo.pageTotal > selectPageInfo.pageIndex) {
                const data = {
                  pageIndex: selectPageInfo.pageIndex + 1,
                  pageSize: selectPageInfo.pageSize,
                  orgId,
                  nameOrPhone: inputValue,
                };
                postChangeAdminList(data).then((res) => {
                  if (res.code == 0) {
