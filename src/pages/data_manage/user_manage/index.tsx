...
import { Button, Col, Form, Input, Row, message as antdMessage, DatePicker, Modal, Select} from 'antd';
import SelfTable from '@/components/self_table';
import UploadModal from './components/uploadModal';
import { getUserList, deleteByIds } from '@/services/data-manage';
...
const sc = scopedClasses('user-manage');
...
export default () => {
  ...
  const getPage = async (data: any) => {
    setLoading(true);
    try {
      const { result, totalCount, pageTotal, code, message } = await getUserList(data);
      setLoading(false);
      if (code === 0) {
        ...
        return (
          <>
            <Button
              ...
              type="link"
              onClick={() => {
                history.push(`${routeName.DATA_MANAGE_USER_MANAGE_DETAIL}?record=${JSON.stringify(record)}`);
              }}
            >
              详情
            </Button>
    ...
    return (
      <div className={sc('container-search')}>
        <Form form={searchForm}>
          <Row>
            <Col span={8}>
              <Form.Item labelCol={{ span: 6 }} name="name" label="姓名">
                <Input placeholder="请输入" maxLength={35} />
              </Form.Item>
            </Col>
            <Col span={8}>
              <Form.Item labelCol={{ span: 6 }} name="phone" label="手机号">
            ...
            <Col span={8}>
              <Form.Item labelCol={{ span: 6 }} name="time" label="注册时间">
