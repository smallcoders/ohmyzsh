...
export async function getIndustryTopicData(industry: string) {
  return request<DataColumn.ResultList>(`/antelope-science/mng/industryData/list?industry=${industry}`);
}
...
export async function addIndustryTopic(data?: any) {
  return request<{
    result: any[]
  } & Common.ResultCode&Common.ResultPage>('/antelope-science/mng/industryData/page', {
    method: 'post',
    data,
  });
}
